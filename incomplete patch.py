...
class tail(object):
	...
	def kill(self,snake_part):
		if snake_part[0] <= self.tail[0] + 2 and snake_part[1] <= self.tail[1] + 2 and snake_part[0] >= self.tail[0] - 2 and snake_part[1] >= self.tail[1] - 2 :
			return True
		else:
			return False
...
def new_food():
	display()
	food[0]=random.randint(2*pixel,1000-pixel*2)
	food[1]=random.randint(2*pixel,800-pixel*2)
	pygame.draw.circle(screen, LIGHTBLUE, (food[0], food[1]), pixel)
	pygame.display.update()
