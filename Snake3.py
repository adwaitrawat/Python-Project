...
class tail(object):
	...
	#function to change location of every tail object
	def position(self,i):
		for j in range(0,2):
...
#function responsible for display of everything on the screen
def display():
	pygame.init()
	screen.fill(BLACK)
...
#function which takes user input from keyboard
def direction():
...
#fuction for displaying text in the game
...
#function for killing the snake if it bites itself
def kill():
	if speed[0] == 0 :
		for i in range(1,len(tails)):
...
#function to increase speed of snake
def speed_update(): 
	fps[0] += 2

#function to detect if food is eaten or not and then do things accordingly
def eat():
...
#function to assign new location of food
def new_food(i,j):
	food[0] = i
	food[1] = j
	pygame.draw.circle(screen, LIGHTBLUE, (food[0], food[1]), pixel)
...
#function to update and manage score board 
def score():
...
#the main body of the program where everything is executed
score()
new_food(10*random.randint(2*pixel/10,10*(width//100)-(pixel*2/10)),10*random.randint(2*pixel/10,(40*height//500)-(pixel*2/10)))
while True :
	display()
